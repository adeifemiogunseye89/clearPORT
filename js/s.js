...
async function init() {
  ...
  try {
    ...
    if (res.status === 404) {
      ...
      renderError('err', 'Something went wrong', data.error || 'Please try again in a moment.');
    }
  } catch (err) {
    renderError('err', 'Connection problem', 'Could not check your link — check your connection and reload the page.');
  }
}
